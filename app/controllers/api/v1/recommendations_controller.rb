    ...
    if saving.avatar.blank?
      recommendations << {
        ...
        user.expenses.each do |expense|
          ...
          monthly_income = 0
          monthly_expense = Expense.where(user_id: user.id).where("created_at >= ?", Date.today.beginning_of_month).sum(:cost)
          user_income = Income.where(user_id: user.id)
            user_income.each do |income|
                monthly_income += IncomeData.where(income_id: income.id).sum(:amount)
                end
            if monthly_expense > monthly_income
                recommendations << {
                  category: 'Expenses',
                  recommendation: "Your monthly expenses exceed your income. Review your expenses and consider reducing unnecessary spending."
                }
            end
            if expense.category.blank?
                recommendations << {
                  category: 'Expenses',
                  recommendation: "The category for expense '#{expense.name}' is not specified. Categorize your expenses to better track your spending."
                }
            end
